  ...
  var rendering = () => {
    ...
    for ( s in data ) {
      ...
      for( z in so.sns ){
        zo = so.sns[ z ];
        tags.push(
          <div class="mr-2 mb-2 text-xs inline-flex items-center font-bold leading-sm uppercase px-2 py-2 rounded-full bg-white text-gray-700 border">
...
*/
      result.push(
        <div key={s} href="#" className="group">
        <Link href={"/artist/" + s } target="_blank">
        <div className="aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-lg bg-gray-200 xl:aspect-h-8 xl:aspect-w-7">
          <img src={so.imgUrl} alt={so.names.EN} className="h-full w-full object-cover object-center group-hover:opacity-75"/>
        </div>
        <p className="mt-4 text-lg font-medium text-gray-900">{so.names.KO} - {so.names.EN}</p>
        <h3 className="flex flex-wrap mt-1 text-sm text-gray-700">{tags}</h3>
        </Link>
      </div>
